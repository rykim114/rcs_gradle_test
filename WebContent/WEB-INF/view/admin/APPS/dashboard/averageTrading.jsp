		...
		<div class="card card-custom card-stretch rounded-0">
			...
			<div class="card-header align-items-center">
				<h3 class="card-title align-items-center">
					<span class="font-weight-bolder text-dark">평균매매가</span>
				</h3>
									...
									<div class="form-horizontal" data-ui-search-detail>
										...
										<div class="form-group">
											...
											<div class="col-11 radioWrap large">
												...
												<label><input type="radio"  name="areaStandard" value="rent_ca"  /> 계약면적</label>
											</div>
										</div>
										<div class="form-group">
										...
										<div class="form-group">
											...
											<div class="col-11 radioWrap large" data-search-wrapper="floorBound">
												...
												<label><input type="checkbox" name="floorBound" value="B1F" /> B1 이하</label>
												...
												<label><input type="checkbox" name="floorBound" value="4F이상" /> 4F 이상</label>
								 			</div>
										</div>    
										<div class="form-group">
						...
						<div class="detailContent">
							...
							<div class="fixedWrap" data-ui-user-date="false">
								<div class="d-flex justify-content-between align-items-center h-40px">
									<h3 class="mb-5 changeTitle" data-select-sgg>
										<span class="font-weight-bold text-dark"><span data-sgg-nm=""></span> <span data-search-column-text></span> 평균 매매가 추이</span>
										<i class="fa fa-question-circle icon-md ml-3 tooltipCustom">
											<span class="tooltipCustom-text">
												부동산R114가 자체 분류한 8가지 상가유형에 따라 연도별, 면적별 매매가 정보를 제공합니다. <br>
												매매매물 정보를 바탕으로 매매가를 합산하여 면적 합산수치와 나눠 지역별, 면적별, 층별 평균 매매가 정보를 제공합니다
							...
							<div class="col-6">
								<div class="chartWrap border height-600px" data-chart-wrapper="chart_1_1">
									<h4>
										<span data-sgg-nm></span> <span data-dong-nm></span>  <span data-search-column-text></span> 평균매매가 추이<span data-search-unit-text></span>
									</h4>
			...
			<!-- 							</div> -->
							...
							<div class="col-6">
								...
								<div class="chartWrap height-300px" data-chart-wrapper="chart_2_1">
									<h4>
										<span data-last-year></span> <span data-sgg-nm></span> <span data-dong-nm></span> <span data-search-column-text></span> 평균매매가 추이 - 선택 기간으로부터 최근 5건 <span data-search-unit-text></span>
									</h4>
									<div class="chart">
			 					...
			 					<div class="chartWrap bg-lightgray height-300px">
			 						<div style="position: relative;">
										<div class="all" data-chart-wrapper="chart_3_1" style="display: none;">
											<h4>
												<span data-last-year></span> <span data-sgg-nm></span> <span data-dong-nm></span> <span data-search-column-text></span> 평균 매매가<span data-search-unit-text></span>
											</h4>
...
--%>											
					...
					<div class="position-relative pb-10 " data-table-wrapper="table_5_1" data-ui-user-date="false"><!-- 230824수정 -->
						<div class="d-flex justify-content-between align-items-center h-40px">
							<h3>
								<span class="font-weight-bold text-dark">평균매매가 추이 데이터</span><span data-search-unit-text></span>
								<i class="fa fa-question-circle icon-md ml-3 tooltipCustom">
									<span class="tooltipCustom-text">
				...
				<div class="position-relative pb-10 mb-10" data-table-wrapper="table_5_3" style="display: none;" data-ui-user-date="true">
					<div class="d-flex justify-content-between align-items-center h-40px">
						<h3>
							<span class="font-weight-bold text-dark">평균매매가 추이 평균 데이터</span><span data-search-unit-text></span>
				...
				<div class="position-relative pb-10 mb-10" data-table-wrapper="table_5_2" style="display: none;">
					<div class="d-flex justify-content-between align-items-center h-40px">
						<h3>
							<span class="font-weight-bold text-dark">평균매매가 추이 상세 데이터</span>
							<i class="fas fa-question-circle icon-md text-muted ml-3" data-toggle="tooltip" data-placement="right" title="선택지역의 상가 유형별  평균매매가를 그래프 및 테이블 형식으로 제공"></i>
						</h3>
					</div>
					<div class="mb-2">
						<div class="row align-items-end">
...
<c:out value="<script type='text/template' id='tmplTableaverageTradingSanggaType'>" escapeXml="false" />
{{#each dataArr}}
	<tr class="tr-border data-show maintotal" data-dong="{{dongnm}}">
		<td rowspan="{{../dongRowSpan}}">{{dongnm}}</td>
		<td style="text-align:center;" colspan="2" class="relative2"><span></span>지역별 평균매매가</td>
		{{#each totalAVG}}
...
<c:out value="<script type='text/template' id='tmplTableaverageTradingSanggaType2'>" escapeXml="false" />
{{#each dataArr}}
	<tr class="tr-border data-show maintotal" data-dong="{{dongnm}}">
		<td rowspan="{{../dongRowSpan}}">{{dongnm}}</td>
		<td style="text-align: center;" colspan="2" class="relative2"><span></span>지역별 평균 매매가</td>
		{{#each dongAvg}}
			<td>{{this}}</td>
		{{/each}}
	</tr>
	{{#each ../sanggaTypeArr}}
		<tr class="tr-border data-show subtotal" data-dong="{{../dongnm}}" data-sangga="{{this}}">
			...
			<td style="text-align:center;"><span data-sanggaArea-text></span>평균 매매가</td>
				{{#with (lookup ../this.sanggaData @index)}}
					{{#each this.sanggaAvg}}
...
<c:out value="<script type='text/template' id='tmplTablaverageTradingDefault'>" escapeXml="false" />
{{#each dataArr}}
	<tr class="tr-border data-show maintotal" data-dong="{{dongnm}}">
		<td rowspan="{{../dongRowSpan}}">{{dongnm}}</td>
		<td style="text-align: center;" colspan="2" class="relative2"><span></span>지역별 평균 매매가</td>
...
<c:out value="<script type='text/template' id='tmplTableaverageTradingDetail'>" escapeXml="false" />
{{#each dataArr}}
...
<c:out value="<script type='text/template' id='tmplTableaverageTradingSum'>" escapeXml="false" />
{{#each dataArr}}
	<tr>
		<td>{{dongnm}}</td>
		<td>{{value}}</td>
