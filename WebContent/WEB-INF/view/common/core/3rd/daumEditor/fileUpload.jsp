...
//		$("#arrFileUrl",opener.document).val(fileInfo.imageurl);
		var inputs = '<li>';
		inputs += '<input type="hidden" name="'+ targetId +'_imageurl" value="'+ fileInfo.imageurl +'" />';
		inputs += '<input type="hidden" name="'+ targetId +'_filesize" value="'+ fileInfo.filesize +'" />';
		inputs += '<input type="hidden" name="'+ targetId +'_filename" value="'+ fileInfo.filename +'" />';
		inputs += '<input type="hidden" name="'+ targetId +'_originalPath" value="'+ fileInfo.upload_path +'" />';
		inputs += '<input type="hidden" name="'+ targetId +'_originalName" value="'+ fileInfo.originalName +'" />';
		inputs += '<input type="hidden" name="'+ targetId +'_originalNameExtension" value="'+ fileInfo.originalNameExtension +'" />';
		inputs += '<img src="'+ fileInfo.imageurl +'" width="138px" height="116px" alt="'+fileInfo.originalName+'">';
		inputs += '<a class="btn-delete" onclick="fn_fileDel(this)">삭제</a>';
		inputs += '</li>';

		$("#"+targetId+"_fileList", opener.document).append(inputs);
		closeWindow();
	}
			...
			<dl class="alert">
				...
				<dd>
					<form id="daumEditorForm" enctype="multipart/form-data" method="post" action="">
						<input type="hidden" name="fileReadExt" value="jpg,gif,png,bmp" />					
						<!-- 파일첨부 -->
						<div class="file">
