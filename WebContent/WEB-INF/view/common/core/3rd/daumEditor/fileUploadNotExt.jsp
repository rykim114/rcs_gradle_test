...
<head>
	...
	$(document).ready(function (){
		...
		$('.submit a').on('click', function (){
			...
			form.ajaxSubmit({
				type: 'POST',
				url: '${SSL_URL}/hp/daumEditor/singleUploadNotExtAjax.do',
	...
	function done(fileInfo) {	 //fileInfo는 Ajax 요청 후 리턴하는 JSON형태의 데이터를 담을 객체
		var targetId = "${reqVo.targetId}";
						
		if (typeof(execAttach) == 'undefined') { //Virtual Function
	        return;
	    }
				
//		$("#arrFileUrl",opener.document).val(fileInfo.imageurl);
		var inputs = '<li>';
		inputs += '<input type="hidden" name="i'+targetId+'_arrImageurl" value="'+ fileInfo.imageurl +'" />';
		inputs += '<input type="hidden" name="i'+targetId+'_arrFilesize" value="'+ fileInfo.filesize +'" />';
		inputs += '<input type="hidden" name="i'+targetId+'_arrFilename" value="'+ fileInfo.filename +'" />';
		inputs += '<input type="hidden" name="i'+targetId+'_arrOriginalPath" value="'+ fileInfo.upload_path +'" />';
		inputs += '<input type="hidden" name="i'+targetId+'_arrOriginalName" value="'+ fileInfo.originalName +'" />';
		inputs += '<input type="hidden" name="i'+targetId+'_arrOriginalNameExtension" value="'+ fileInfo.originalNameExtension +'" />';
		
		inputs += '<a href="'+ fileInfo.fileurl +'" alt="'+fileInfo.originalName+'">'+fileInfo.originalName+'</a>';
		inputs += '<a class="btn-delete" onclick="fn_fileDel(this)">삭제</a>';
		inputs += '</li>';
		...
		$("#"+targetId+"fileList",opener.document).append(inputs);
		closeWindow();
	}
	...
	function validation(fileName){
		...
		if( ( (fileNameExtension=='asp') || (fileNameExtension=='html') || 
				(fileNameExtension=='htm') || (fileNameExtension=='php') || 
				(fileNameExtension=='php3') || (fileNameExtension=='cgi') || 
				(fileNameExtension=='phtm') || (fileNameExtension=='phtm') || 
				(fileNameExtension=='inc') || (fileNameExtension=='sql') || 
				(fileNameExtension=='pl') || (fileNameExtension=='jsp') || 
				(fileNameExtension=='js') || (fileNameExtension=='css')
			) ) {
			alert('허용되지 않는  확장자의 파일입니다.');
			return true;
		}else{
				...
				<dd>
					<form id="daumEditorForm" enctype="multipart/form-data" method="post" action="">
						...
						<div class="file">
							...
							<label class="file-btn" for="uploadInputBox">파일선택</label>
