...
var apiSearchAreaMap = function() {
	...
	var fnUpdateSggList = function(sidonm, sidocd, sggnm, sggcd, clickobj) {
		...
		return fnListPrivate({
			...
			var sggArrList = JSON.parse(JSON.stringify(sggArr));
			if(!chkbizdist){
			...
			if(chkbizdist){
				...
				if(isfirst == true) {
					...
					sggnm = sggArrList[1].sggnm;
					fnUpdateAreaList(sggnm, sggArrList[1].adm_cd, "", "", "", clickobj);
				}
			}
	...
	var fnUpdateAreaList = function(sggnm, sggcd, areanm, areacd, geom, clickobj) {		
		...
		if(chkbizdist){
			...
			return fnListPrivate({
				...
				var dongArrList = JSON.parse(JSON.stringify(dongArr));
