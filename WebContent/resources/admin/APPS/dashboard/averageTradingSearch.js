...
var apiSearchaverageTrading = function() {
    // Private functions
	var isActivated,
		$btnActivate,
		$searchWrapper,
...
//		});
    ...
    return {
        ...
        init: function(options) {
			...
			$radioResultArea = $searchWrapper.find('[type=radio][name=areaStandard]');
...
//			}
			...
			var searchDtl = {
					...
					checkAreaBound: [],
					checkFloorBound: [],
					radioResultArea: $searchWrapper.find('[type=radio][name=areaStandard]:checked').val(),
					isStartDate: true,
			...
			if ($searchWrapper.find('[type=radio][name=priceUnit]:checked').val() === "py") {
				$('[data-search-price-text]').text('만원 / 3.3㎡');				
			} else {
				$('[data-search-price-text]').text('만원 / ㎡');
			}
