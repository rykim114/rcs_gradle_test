...
var apiSupplyTrend = function() {
	...
	var chartSupplyTrendAll = {
		...
		loadNextObj: function(selected) {
			...
			chartSupplyTrendYear.init(self.param, self.searchDtl);
		},		
		...
		loadData: function() {
			...
			return $.when(
				z.xAsync('SupplyTrend', '공급추이차트', 'select', param, 'json'),
				z.xAsync('SupplyTrend', '공급추이차트_누적', 'select', paramAcc, 'json')
		...
		loadAxis: function() {
			...
			for (var i in rawDataArr) {
				var raw = rawDataArr[i],
					mmt = moment(raw['분양일'], 'YYYYMM');

				switch (timeBound) {
					case 'year':
						raw['분양일'] = mmt.format('YYYY');
						break;
					case 'month':
						raw['분양일'] = mmt.format('YYYY.MM');
						break;
					case 'half':
						raw['분양일'] = mmt.format('YYYY.') + (6 > mmt.get('month') ? '1' : '2') + 'H';
						break;
					case 'quarter':
						raw['분양일'] = mmt.format('YYYY.Q[Q]');
						break;
				}
			}
...
//			self.yAxisArr = chartSeries;
		...
		updateDataByYear: function(chartOptions, chartSeries) {
			...
			for (var i in rawDataArr) {
				var raw = rawDataArr[i],
					idx = self.xAxisArr.indexOf(raw['분양일']),
	...
	var chartSupplyTrendYear = {
		...
		loadNextObj: function(selected) {
			...
			chartSupplyTrendDong.hide();
			chartSupplyTrendSanggaType.hide();

			if ('전체' === self.param.dongnm) {
				chartSupplyTrendDong.init(self.param, self.searchDtl);
			} else {
				chartSupplyTrendSanggaType.init(self.param, self.searchDtl);
			}
		},
		...
		loadData: function() {
			...
			return z.xAsync('SupplyTrend', '공급추이차트', 'select', param, 'json').done(function(resp) {
				self.rawDataArr = resp;
...
//			self.yAxisArr = chartSeries;
		...
		updateDataByYear: function(chartOptions, chartSeries) {
			...
			if ('area' === self.columnMode) {
				...
				for (var i in rawDataArr) {
					var raw = rawDataArr[i];
					var idxYear = self.xAxisArr.indexOf(raw['분양일']);
					if (-1 < idxYear) {
	...
	var chartSupplyTrendDong = {
		...
		loadData: function() {
			...
			return z.xAsync('SupplyTrend', '공급추이차트', 'select', param, 'json').done(function(resp) {
				...
				for (var i in resp) {
					var raw = resp[i],
						ymd = 4 === raw['분양일'].length ? raw['분양일'] + '01' : raw['분양일'],
...
//			self.yAxisArr = chartSeries;
	...
	var chartSupplyTrendSanggaType = {
		init: function(param, searchDtl) {
			var self = this,
				apexChart = '#chart_3_2';
...
//			self.yAxisArr = chartSeries;
	...
	// 공급 추이 상가유형 데이터
	var tableSupplyTrendSanggaType = {
		init: function(param, searchDtl) {
			var self = this;
		...
		setBtnListener: function() {
			...
			self.$wrapper.find('.allView2').off('click').on('click', function() {
				tableSupplyTrendDetail.openAllView();
			});
		},
		loadData: function() {
			var self = this,
		...
		updateDataBySanggaType: function() {
			...
			if(self.param.sidonm == '전국') {
				...
				for (var i in rawDataArr) {
					...
					// 미포함 데이터 제외
					if (0 > idxSido || 0 > idxData) {
						continue;
					}

					var sidoData = resultArr[idxSido],
						idxYear = self.xAxisArr.indexOf(rawData['분양일']);
		...
		updateData: function() {
			var self = this,
				tmpl = Handlebars.compile($('#tmplTableSupplyTrendSanggaType').html()),
		...
		exportSheet: function() {
			...
			setTimeout(function() {
				...
				for (var i in self.rawDataArr) {
					...
					rowData[9] = self.rawDataArr[i]['분양년도'];
	...
	var tableSupplyTrendSum = {
		...
		updateData: function(isAppendHidden) {
			var self = this,
				tmpl = Handlebars.compile($('#tmplTableSupplyTrendSum').html()),
				rawDataArr = self.rawDataArr,
				dataArr = [],
				dongArr = [],
				sidoArr = [],
    ...
    return {
        ...
        init: function() {
			...
			chartSupplyTrendAll.init();
			chartSupplyTrendYear.init();

			chartSupplyTrendDong.init();
			chartSupplyTrendSanggaType.init();
		...
		setBtnListener: function() {
			$('[data-btn-download="table_5_1"]').click(function() {
				...
				$.when(
					tableSupplyTrendSanggaType.exportSheet()
			...
			$('[data-btn-dtldownload="rowDataTable"]').click(function() {
				...
				$.when(
					tableSupplyTrendDetail.exportSheet()
				).done(function(workSheet) {
					var wb = XLSX.utils.book_new();
			...
			$('[data-btn-download="table_5_3"]').click(function() {
				...
				$.when(
					tableSupplyTrendSum.exportSheet()
				).done(function(workSheet) {
					var wb = XLSX.utils.book_new();
		...
		setDongCd: function(param) {
			...
			if ('custom' === searchDtl.radioTimeBound) {
				...
				tableSupplyTrendSanggaType.init(param, searchDtl);
				tableSupplyTrendDetail.init(param, searchDtl);
			}
		}
    };
...
$(function() {
	$.when(
		...
		$.getScript('/resources/admin/APPS/dashboard/supplyTrendSearch.js'),
		$.getScript('/resources/common/custom/js/commonDashboard.js')
	).done(function() {
		
		z.xAsync('AdminMain', 'getExcelDown', 'select', {pgmCode:"MA0103"}, 'json').done(function(resp) {
		...
		apiSearchAreaMap.init({
			...
			areaCdListener: apiSupplyTrend.setDongCd
		});

		apiSearchEmd = apiSearchAreaMap;

		apiSearchSupplyTrend.init({
