...
var apiSearchSupplyTrend = function() {
    // Private functions
	var isActivated,
		$btnActivate,
		$searchWrapper,
    ...
    return {
        ...
        init: function(options) {
			...
			//REF : DB에서 읽어오는것에서 오늘날짜 + 1년으로 변경처리
			/*
			z.xAsync('SupplyTrend', '공급최종일자', 'select', '', 'json').done(function(resp){
				var maxdt = resp;
			});
			*/
			//console.log(moment());
			//console.log(moment().format('YYYY-MM-DD'));
			//console.log(moment().clone().subtract(5, 'years'));
			var now = endDt,
				pre = now.clone(),
