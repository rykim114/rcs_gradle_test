var building_apiAddr = (function() {
	...
	var LICENSE_KEY = '14cc08123cd6d425d603917caf3ee061895e4192',
		...
		$addrWrapperb = $('#addrWrapperb'),
		$listSido = $addrWrapperb.children().eq(0),
		$listSgg = $addrWrapperb.children().eq(1),
		$listDong = $addrWrapperb.children().eq(2),
		$btnSearchAddr = $('#btnSearchAddr'),
		$btnSearchAddrSido = $btnSearchAddr.find('[data-addr-sidob]'),
		$btnSearchAddrSgg = $btnSearchAddr.find('[data-addr-sggb]'),
		$btnSearchAddrDong = $btnSearchAddr.find('[data-addr-dongb]'),
		$divSearchAddr = $('#divSearchAddr'),
		$divSearchAddrSido = $divSearchAddr.find('[data-addr-sidob]'),
		$divSearchAddrSgg = $divSearchAddr.find('[data-addr-sggb]'),
		$divSearchAddrDong = $divSearchAddr.find('[data-addr-dongb]'),
		...
		$addrSearchText = $('#wrap [data-addr-search-text]'),
		$btnClearSearchText = $('#wrap [data-btn-clear-addr]'),
	...
	var fnUpdateAddr = function(addr) {
		...
		$listSgg.find('ul a').removeClass('active');
		$listSgg.find('[data-list-addr-sgg="' + addr.sggnm + '"]').addClass('active');
		...
		ls_sidonm	=	addr.sidonm;
		ls_sggnm	=	addr.sggnm;
		ls_dongnm	=	addr.dongnm;		
	};
	...
	var fnMoveMap = function(x, y, markerVis) {		
		if (! apiMap || ! x || ! y) {
			return;
		}
		
		apiMap.panTo(new L.LatLng(y, x));
