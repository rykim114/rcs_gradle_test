...
//var chkTakji = ["UDA100","UDT100","UDT101","UDT102","UDT103","UQQ300","UQQ310","UQQ320","UDS100","UDR100"]; // 개발지구, 레벨별 다르게
...
//왼쪽 탭 버튼
var arrTab = new Array('SearchTab', 'NaviTab', 'ThemeTab', 'LifeTab', 'StatsTab', 'FavoriteTab');
...
/*	map.bizdistArr = [];
	...
	/*$('.btn-toggle-bizdist').click(function() {
	...
	//그래프 리사이징
	$('#mapPannel1 [data-toggle="tab"]').click(function() {
//		var $panel = $('#mapPannel1');
//				
...
//		}, 160);		
	...
	$('#mapPannel1_2 [data-toggle="tab"]').click(function() {
//		var $panel = $('#mapPannel1_2');
//		
//		$panel.attr('style', 'width: 599px !important');
//		setTimeout(function() {
//			$panel.attr('style', 'width: 601px !important');
...
//		}, 160);
	...
	$('#mapPannel2 [data-toggle="tab"]').click(function() {
//		var $panel = $('#mapPannel2');
//				
//		$panel.attr('style', 'width: 599px !important');
//		setTimeout(function() {
//			$panel.attr('style', 'width: 601px !important');
...
/*	var $scroll = $('#mapPannel1 .ps.scroll');
...
//	});
	...
	map.on("drag",function(e) {
		if(map.getZoom() == 11){	
			...
			if (map.popupList && 0 < map.popupList.length && 150 < map.popupList.length) {			
...
/*	if(mapCopyType!="") {
		...
		*//** 190829_JIK **//*
		if(mapCopyType == "grid"){ // 지도 복사 시 타입이 격자 일때
			var coords = mapCopyData2.split("_"); // 지도 복사 할 때 입력 된 좌표
			map.setView(new L.LatLng(coords[0], coords[1]) , 12); //지도 위치 이동  (좌표, 지도 레벨)
...
/*	var constant = carRouteInfos.body.constant;
	for(var i=0; i<constant.length; i++) {
		...
		if(code!=undefined && code!=null) {
			for(var j=0; j<code.length; j++) {
				if(lan=="KOR") {
					carRouteContent.set(desc+"_"+code[j].code,code[j].korDesc);
				}else if(lan=="ENG") {
					carRouteContent.set(desc+"_"+code[j].code,code[j].engDesc);
				}else if(lan=="JAN") {
					carRouteContent.set(desc+"_"+code[j].code,code[j].jpnDesc);
				}else if(lan=="CHINAG") {
					carRouteContent.set(desc+"_"+code[j].code,code[j].chnDesc);
				}
			}
		}
	}*/
...
/*	if($("#param_lat").val() != ""){
		getBuildingPoi($("#param_lat").val(),$("#param_lng").val())
	}else{
...
function notice() {
	if($("#noticeBg").css("display")=="none") {
		$("#headerRightForm").css("display","none");
		$("#noticeBg").css("display","block");
	}else {
		//포팅시 URL 수정
		document.getElementById("ifrm1").src = "https://pns.dawulmap.com/customer2/notice/notice.php?type=B0002";
		//document.getElementById("ifrm1").src = "/resources/common/custom/customer2/notice/notice.php?type=B0002";
		//document.getElementById("ifrm1").src = "http://222.239.253.18/dawulmap/customer2/notice/notice.php?type=B0002";
		$("#headerRightForm").css("display","");
...
function languageListHover() {
	...
	/*if($("#language_list_sub").css("display")=="none") {
		$("#language_tarrow").attr("class","l_tarrow");
		$("#language_list_sub").css("display","block");
		$("#language_list_sub").animate({
		    height: "65px"
	  	}, 200 );
	}else {
		$("#language_list_sub").animate({
		    height: "0px"
	  	}, 200 , languageListCallBack);
	}*/
}

//지도 언어변경 드롭다운 콜백
/*function languageListCallBack() {
	$("#language_list_sub").css("display","none");
	$("#language_tarrow").attr("class","l_barrow");
}*/
...
/*function measureHover() {
	if(!chkDist) {
		if($("#measureOff").css("display")=="none") {
			$("#measureOff").css("display","block");
			$("#measureOn").css("display","none");
		}else {
			$("#measureOff").css("display","none");
			$("#measureOn").css("display","block");
		}
	}
}*/
...
function ncodeClickSubDepth2(selectIndex) {	
	...
	function sample_marker(neLat,neLng,swLat,swLng,ls_year_length){ // 마커 설정
		...
		param = {
			...
			if(ls_success + ls_warning + ls_primary == "000" || ls_year_length == ''){
			...
			if(map.getZoom() > 10 && ls_year_length != '' && ls_success + ls_warning + ls_primary != "000"){
				/*분양가*/
				return z.xAsync('dawulmap', 'zeons_마커생성', 'select', param, 'json2').always(function() {
					...
					if(resp[i].z_status == "분양"){
						...
						html_ += resp[i].임대료명;
						html_ += "<div class='flag-body popupopen' onclick=getPnu('" + resp[i].x_lat + "','" + resp[i].y_lng + "','" + resp[i].pnu + "','" + resp[i].xy + "','" + resp[i].z_status + "','" + resp[i].bunji + "')>";
						
						if(resp[i].z_status == "분양"){
							html_ += "<strong>분양일</strong>";
						}else if(resp[i].z_status == "임대료"){
							html_ += "<strong>등록일</strong>";
						}else{
							html_ += "<strong>최근거래일</strong>";
						}					
						...
						if(resp[i].z_status == "분양"){
							html_ += "<div class='z_leaflet-popup' id='pop_sale'>";
							...
							html_ += "<strong>분양일 : <span>" + resp[i].w_date + "</span></strong>";
							html_ += "</div>";
							html_ += "<div class='leaflet-body " + resp[i].xy + "' data-scroll='true' data-height='155' data-mobile-height='155' style='height:155px;'>";						
						}else if(resp[i].z_status == "임대료"){
							html_ += "<div class='z_leaflet-popup' id='pop_rent'>";
							html_ += "<div class='z_leaflet-popup-content'>";
							html_ += "<div class='z_leaflet-content-box'>";
							html_ += "<div class='z_leaflet-header'>";
							html_ += "<h3 id=" + resp[i].xy + ">-</h3>";
							html_ += "<strong>등록일 : <span>" + resp[i].w_date + "</span></strong>";
							html_ += "</div>";
							html_ += "<div class='leaflet-body " + resp[i].xy + "' data-scroll='true' data-height='155' data-mobile-height='155' style='height:155px;'>";						
						}else{
							html_ += "<div class='z_leaflet-popup' id='pop_actual'>";
							html_ += "<div class='z_leaflet-popup-content'>";
							html_ += "<div class='z_leaflet-content-box'>";
							html_ += "<div class='z_leaflet-header'>";
							html_ += "<h3 id=" + resp[i].xy + ">-</h3>";
							html_ += "<strong>최근계약일 : <span>" + resp[i].w_date + "</span></strong>";
							html_ += "</div>";
							html_ += "<div class='leaflet-body " + resp[i].xy + "' data-scroll='true' data-height='155' data-mobile-height='155' style='height:155px;'>";						
						}
				...
				}else if(map.getZoom() < 11 && map.getZoom() > 6 && ls_year_length != '' && ls_success + ls_warning + ls_primary != "000"){
					/*분양가*/
				...
				}else if(map.getZoom() < 7 && map.getZoom() > 4 && ls_year_length != '' && ls_success + ls_warning + ls_primary != "000"){
					/*분양가*/
				...
				}else if(map.getZoom() < 5 && map.getZoom() > 1 && ls_year_length != '' && ls_success + ls_warning + ls_primary != "000"){
					/*분양가*/
...
function getPnu(lat,lng,pnu,xy,click_name,bunji) {		
