// silverlightLoader.js
// This file is ported from slcanvas, original file name is slcanvas.js see http://http://slcanvas.codeplex.com/
// It's under Microsoft Public License (Ms-PL), see http://slcanvas.codeplex.com/license
// This script is licensed "as-is." You bear the risk of using it.


window['slcanvas'] = {};

slcanvas.xappath = installPath + 'common/SpreadJS/ClientBin/SxBridge.xap';
...
slcanvas.canvasHtmlEnd =
    '>' +
        '<param name="source" value="' + slcanvas.xappath + '"/>' +
...
slcanvas.createCanvasDiv = function(width, height, action) {
    ...
    div.innerHTML = slcanvas.canvasHtmlStart + " width='" + width + "' height='" + height + "'" + slcanvas.canvasHtmlEnd;
...
slcanvas.autoLoad = function() {
    ...
    for (var i = 0; i < canvasTags.length; i++) {
        ...
        var spreadAttr = canvasTag.getAttribute('gcSpreadsheet');
        if (!spreadAttr) continue;
...
slcanvas.getScriptId = function(tagEl, scriptid) {
    if (!scriptid || scriptid.length == 0) {
        ...
        //        }
        scriptid = "slcanvas_" + tagEl.parentNode.id;
    }
    return scriptid;
};
...
if (window['defaultCanvasRenderMethodToAuto']) {
