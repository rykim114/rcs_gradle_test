		...
		*/
		...
		$("#report").scroll(function(){
			...
			if (jQuery.browser.mozilla) {
				...
				start = parseInt($('#'+ currentPage).offset().top) + parseInt($('#report').scrollTop());
				start = start * zoomRate ;
				if(currentPage == 1)
					start = 0;
				currentPage++;
				...
				if (currentPage > endPage) 
					...
					end   = parseInt($('#'+ currentPage).offset().top) + parseInt($('#report').scrollTop());
					end = end * zoomRate;
				}
				currentPage--;			
			}
			else{
				//alert("page="+currentPage + " start="+start + " end= "+end + " position="+position+" rate= "+zoomRate);
				//alert(currentPage);
				//var start = parseInt($('#'+ 1).offset().top) + $('#report').scrollTop();
				start = parseInt($('#'+ currentPage).offset().top) + $('#report').scrollTop();
				if(currentPage == 1)
					start = 0;
				currentPage++;
				...
				if(currentPage > endPage)
					...
					end   = parseInt($('#'+ currentPage).offset().top) + $('#report').scrollTop();
			...
			if (jQuery.browser.mozilla){ 
				var position =  parseInt($('#report').scrollTop()) + 35 + 25;
				if(zoomRate < 1)
					position =  parseInt($('#report').scrollTop()) + 35 + 1;
		
			}
			else
				var position =  $('#report').scrollTop() + 35 + 1;
			
			//alert("page="+currentPage + " start="+start + " end= "+end + " position="+position+" rate= "+zoomRate);
			//alert("scrollTop= "+($('#report').scrollTop())+ " offsetTop= "+($('#'+currentPage).offset().top));
			if(position < start){
				currentPage--;
				if(currentPage < startPage)
	...
	function goScroll(id){
		...
		if (jQuery.browser.mozilla) {
			var myOffset = parseInt($('#report').scrollTop()) + parseInt($('#' + id).offset().top);
			myOffset = myOffset * zoomRate ;
			
			$('#report').animate({
				scrollTop: parseInt(myOffset)
			}, 0);
			
			
			//$("#page option",parent.document).eq(id-1).attr("selected", "selected");
		}
		else if (jQuery.browser.msie) {
			if(zoomRate != 1.0){
				$('#subreport').animate({
					scrollTop: pageTop[id]
					}, 0);	
				//$("#page option",parent.document).eq(id-1).attr("selected", "selected");
			}
			else{
			...
			$('#report').animate({
				scrollTop: parseInt($('#report').scrollTop()) + parseInt($('#' + id).offset().top)
			}, 0);
			//$("#page option",parent.document).eq(id-1).attr("selected", "selected");
